...
![NVIDIA GB10 — Lenovo ThinkStation PGX](assets/Lenovo-PGX.jpg)
...
No cloud, no API. Every token was generated on the box. The scripts and raw results are in this repo so you can re-run and check the numbers yourself.
...
## Headline results (reasoning off)
...
The same matrix was run with reasoning enabled (`--think`). Full data: [`results/summary-thinkon.md`](results/summary-thinkon.md).
...
## Embedding models - Czech RAG

Retrieval quality on a small synthetic Czech corpus (16 passages, 12 paraphrase queries - semantic match, not keyword overlap). Full data: [`results/embed.md`](results/embed.md).
